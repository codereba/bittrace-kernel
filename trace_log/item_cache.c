	...
	do 
	{
		...
		for( i = 0; ( ULONG )i < table->size; i ++ )
		{
			...
			for( ; ; )
			{
				...
				{
					NTSTATUS _ntstatus; 
					//if( FALSE == IsListEmpty( &data->cache_entry ) )
					//{
					//	ASSERT( FALSE ); 
					//}

					//dbg_print( MSG_INFO, "delete the cache data that reference count is %d\n", data->ref_count ); 
...
INLINE ULONG calc_str_hash_code( LPCWSTR str, ULONG len, ULONG tbl_size )
{
	ULONG hash_code = 0; 
	LPCWSTR file_name; 
	ULONG index; 
	...
	do 
	{
		...
		{
			if( file_name[ index ] == L'\0' )
			{
				break; 
			}

			upper_ch = file_name[ index ]; 

			if( upper_ch >= L'a' && upper_ch <= L'z' )
			{
				upper_ch = upper_ch + ( L'A' - L'a' ); 
			}

			hash_code += ( upper_ch << 2 ); 
		}
