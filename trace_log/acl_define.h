...
//#ifndef _MAX_FILE_NAME_LEN
//#define _MAX_REG_PATH_LEN 1024 //2048
//#endif //_MAX_FILE_NAME_LEN
...
//( type == COM_DEFINE || type == REG_DEFINE || type == FILE_DEFINE || type == URL_DEFINE ) 
...
//typedef struct _sys_action_info_notify
//{
...
//
//} sys_action_info_notify, *psys_action_info_notify; 
...
typedef struct _r3_action_notify
{
	...
	ULONG frame_count; 
	/**************************************************************
	if ring 3 is 32bit, but ring 0 is 64bit, then failed.
	**************************************************************/
...
//FIELD_OFFSET( sys_action_info, action ) + 
...
//		ip_param_define ip; 
//	};
//} param_ip_desc, *pparam_ip_desc; 
//
//typedef struct _param_port_desc
//{
//	param_define_type type; 
//	ULONG id; 
//	BOOLEAN is_cls; 
...
//		port_param_define port; 
//	};
//} param_port_desc, *pparam_port_desc; 
...
typedef struct _action_desc
{
	...
	//USHORT param_offs[ MAX_RULE_PARAM_NUM ]; 
...
//the process handle is really handle, not ZwCurrentProcess (-1)
#define is_valid_proc_handle( proc_handle ) ( proc_handle != NULL && proc_handle != ZwCurrentProcess() ) 


//INLINE NTSTATUS allocate_relating_data( HANDLE proc_handle/*ULONG protect, */ULONG data_size, data_alloc_mode mode, relating_data *data_buf_out )
//{
...
//	do 
//	{
//		proc_id = PsGetThreadProcessId( cur_thread ); 
//	} while ( FALSE );
//
//	return ntstatus; 
//}
...
FORCEINLINE ULONG calc_action_output_size( ULONG data_len )
{
	...
	//size = sizeof( sys_action_desc ); 
	size = data_len; 
	size += sizeof( sys_action_output ); 

	//size += FIELD_OFFSET( sys_action_output, action ); 
...
//			else if( ( ULONG_PTR )data_cont <= MmUserProbeAddress )
//			{
//#if 0
//				DbgBreakPoint(); 
//#endif //0
...
INLINE NTSTATUS generate_name_action_desc( sys_action_type action_type, LPCWSTR app_name, LPCWSTR param, sys_action_desc *desc )
{
	...
	//desc->resp = ACTION_ALLOW; 
	//desc->id = INVALID_ACTION_ID; 
	//desc->type = action_type; 
	...
	//ntstatus = copy_unicode_string_to_buf( app_name, 
	//	desc->desc.params[ 0 ].define.app.app_name, 
	//	buf_len, NULL ); 
	//if( !NT_SUCCESS( ntstatus ) )
	//{
	//	goto _return; 
