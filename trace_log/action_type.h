...
#ifdef COMPATIBLE_OLD_ACTION_DEFINE
typedef struct _ipv4_socket_action
{
	ULONG src_ip; 
	ULONG dest_ip; 
	USHORT src_port; 
	USHORT dest_port; 
	prot_type prot; 
	socket_action_ret result; 
} ipv4_socket_action, *pipv4_socket_action; 

typedef struct _ipv4_icmp_action
{
	ULONG src_ip; 
	ULONG dest_ip; 
	BYTE type; 
	BYTE code; 
} ipv4_icmp_action, *pipv4_icmp_action; 

typedef struct _access_url_info
{
	LPCSTR domain_name; 
	LPCSTR path_name; 
} access_url_info, *paccess_url_info; 

typedef struct _access_reg_info
{
	//action_access_type access_type; 
	union
	{
		WCHAR path_name[ _MAX_REG_PATH_LEN ]; 
		WCHAR *path_name_ptr; 
	};
} access_reg_info, *paccess_reg_info; 

typedef struct _access_file_info
{
	//action_access_type access_type; 
	union
	{
		WCHAR path_name[ _MAX_FILE_NAME_LEN ]; 
		WCHAR *path_name_ptr; 
	};
} access_file_info, *paccess_file_info; 

typedef struct _manage_driver_info
{
	union
	{
		WCHAR path_name[ _MAX_FILE_NAME_LEN ]; 
...
} manage_driver_info, *pmanage_driver_info;

typedef struct _manage_proc_info 
{
	union
	{
		WCHAR path_name[ _MAX_FILE_NAME_LEN ]; 
...
} manage_proc_info, *pmanage_proc_info; 
